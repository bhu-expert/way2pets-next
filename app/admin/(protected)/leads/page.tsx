...
export default async function Page({ searchParams }: { searchParams?: Promise<{ q?: string; status?: string; new?: string }> }) {
  ...
  const resource = resources.leads
  if (params?.new) return <CmsForm resourceKey="leads" resource={resource} row={null} />
  ...
  return <CmsList resourceKey="leads" resource={resource} rows={filtered} search={params?.q || ''} status={status} />
}
