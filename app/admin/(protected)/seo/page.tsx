...
export default async function Page({ searchParams }: { searchParams?: Promise<{ q?: string; status?: string; new?: string }> }) {
  ...
  const resource = resources.seo
  if (params?.new) return <CmsForm resourceKey="seo" resource={resource} row={null} />
  ...
  return <CmsList resourceKey="seo" resource={resource} rows={filtered} search={params?.q || ''} status={status} />
}
