...
export default async function Page({ searchParams }: { searchParams?: Promise<{ q?: string; status?: string; new?: string }> }) {
  ...
  const resource = resources.redirects
  if (params?.new) return <CmsForm resourceKey="redirects" resource={resource} row={null} />
  ...
  return <CmsList resourceKey="redirects" resource={resource} rows={filtered} search={params?.q || ''} status={status} />
}
